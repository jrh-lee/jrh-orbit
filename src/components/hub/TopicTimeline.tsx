...
export function TopicTimeline({ entries, onOpen }: Props) {
  const [showAll, setShowAll] = useState(false);
  const visible = showAll ? entries : entries.slice(0, PAGE_SIZE);

  return (
    <section>
      <h2 className="text-sm font-semibold text-ink px-3 mb-2">타임라인</h2>
      {visible.length === 0 ? (
        <p className="text-xs text-ink-3 px-3">항목이 없습니다.</p>
      ) : (
        <div className="space-y-0.5">
          {visible.map((e, i) => (
            <TimelineItem key={`${e.noteId}-${i}`} entry={e} onOpen={onOpen} />
          ))}
        </div>
      )}
